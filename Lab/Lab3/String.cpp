...
String& String::erase(size_t pos, size_t len) {
    ...
    if (len == npos || pos + len >= sz){
        char *sec_ptr = &cstr[pos];
        *sec_ptr = a_null_byte;
        sz = pos;
        return *this;
    }
    char *fir_ptr = &cstr[pos];
    char *sec_ptr = &cstr[pos + len];
...
String& String::insert(size_t pos, const String& str) {
    ...
    if (pos != sz){
        for (size_t i = 1; i <= sz - pos; i++){
            cstr[sz + str.size() - i] = cstr[sz - i];
        }
    }
...
size_t String::find_last_of(const String& str, size_t pos) {
    ...
    for (size_t i = std::min(pos, sz - 1); i > 0; i--){
        for (size_t j = 0; j < str.size(); j++){
            if (str[j] == cstr[i]){
                return i;
            }
