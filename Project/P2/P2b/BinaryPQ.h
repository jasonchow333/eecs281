...
public:
    ...
    BinaryPQ(InputIterator start, InputIterator end, COMP_FUNCTOR comp = COMP_FUNCTOR()) :
        ...
        std::vector<TYPE> temp(start, end);
        this->data.swap(temp);
        updatePriorities();
    } // BinaryPQ
...
private:
    ...
    void fixUp(std::size_t idx){
        while (idx > 0 && this->compare(this->data[(idx - 1) / 2], this->data[idx])){
            std::swap(this->data[idx], this->data[(idx - 1) / 2]);
            idx = (idx - 1) / 2;
        }
    }

    void fixDown(std::size_t idx){
        while (2 * idx + 1 < size()){
            std::size_t j = 2 * idx + 1;
            if (j < size() - 1 && this->compare(this->data[j], this->data[j + 1])) ++j;
            if (!this->compare(this->data[idx], this->data[j])) break;
            std::swap(this->data[idx], this->data[j]);
            idx = j;
        }
    }
}; // BinaryPQ
